...
public class PetriSim implements Cloneable, Serializable {
   ...
   public PetriSim(PetriNet net) {
     ...
     */
    public ArrayList<PetriT> findActiveT(double currentTime, Consumer<String> logAction) {
        ArrayList<PetriT> aT = new ArrayList<PetriT>();

        for (PetriT transition : listT) {
            if ((transition.condition(listP, logAction) == true) && (transition.getProbability() != 0)) {
                aT.add(transition);
     ...
     * Do one event
     */
    public void step(Consumer<String> logAction) { //один крок ,використовується для одного об'єкту мережа Петрі

        System.out.println("Next Step  " + "time=" + this.getCurrentTime());

        this.printMark();//друкувати поточне маркування
        ArrayList<PetriT> activeT = this.findActiveT(this.getCurrentTime(), logAction);     //формування списку активних переходів
        ...
        if ((activeT.isEmpty() && isBufferEmpty() == true) || this.getCurrentTime() >= getSimulationTime()) { //зупинка імітації за умови, що
         ...
         //   stop = true;                              // і немає маркерів у переходах, або вичерпаний час моделювання
            ...
            while (activeT.size() > 0) { //вхід маркерів в переходи доки можливо
                ...
                this.doConflikt(activeT, listP, this.getCurrentTime(), logAction).actIn(listP, this.getCurrentTime(), logAction); //розв'язання конфліктів
                
                activeT = this.findActiveT(this.getCurrentTime(), logAction); //оновлення списку активних переходів
            ...
            if (this.getCurrentTime() <= getSimulationTime()) {
                ...
                //Вихід маркерів
                eventMin.actOut(listP, this.getCurrentTime(), logAction);//Вихід маркерів з переходу, що відповідає найближчому моменту часу
                ...
                if (eventMin.getBuffer() > 0) {
                    ...
                    while (u == true) {
                        ...
                        if (eventMin.getMinTime() == this.getCurrentTime()) {
                            ...
                            eventMin.actOut(listP, this.getCurrentTime(), logAction);
                            // this.printMark();//друкувати поточне маркування
                        } else {
                            u = false;
                        }
                ...
                for (PetriT transition : listT) {//ВАЖЛИВО!!Вихід з усіх переходів, що час виходу маркерів == поточний момент час.
                    ...
                    if (transition.getBuffer() > 0 && transition.getMinTime() == this.getCurrentTime()) {
                        transition.actOut(listP,this.getCurrentTime(), logAction);//Вихід маркерів з переходу, що відповідає найближчому моменту часу
                        ...
                        if (transition.getBuffer() > 0) {
                            ...
                            while (u == true) {
                                ...
                                if (transition.getMinTime() == this.getCurrentTime()) {
                                    transition.actOut(listP, this.getCurrentTime(), logAction);
                                    // this.printMark();//друкувати поточне маркування
                                } else {
                                    u = false;
                                }
     ...
     */
    public void input(Consumer<String> logAction) {//вхід маркерів в переходи Петрі-об'єкта
    ...
    public void output(Consumer<String> logAction){
            if (this.getCurrentTime() <= this.getSimulationTime()) {
            eventMin.actOut(listP, this.getCurrentTime(), logAction);//здійснення події
            if (eventMin.getBuffer() > 0) {
                boolean u = true;
                while (u == true) {
                    eventMin.minEvent();
    ...
    public void go() {
        ...
        while (this.getCurrentTime() <= getSimulationTime() && isStop() == false) {
            this.step(s -> System.out.println(s));
            if (isStop() == true) {
                System.out.println("STOP in net  " + this.getName());
            }
        }
     ...
     */
    public PetriT doConflikt(ArrayList<PetriT> transitions, PetriP[] listP, double currentTime, Consumer<String> logAction) {//
        PetriT aT = transitions.get(0);
        List<PetriT> conflictTransitions = transitions;
        if (transitions.size() > 1) {
            if (transitions.stream().allMatch(t -> t.getTransitionType() == TransitionType.WORKBENCH)){
                PetriP queueP = Arrays.stream(listP).filter(PetriP::isJobQueue).findFirst().orElse(null);
                if (queueP == null)
                    return null;
                ...
                for (PetriT transition : transitions) {
                    int priority = transition.getPriority(jobMarkers);

                    if (!priorityMap.containsKey(priority))
                        priorityMap.put(priority, new ArrayList<>());

                    priorityMap.get(priority).add(transition);
                }

                Integer maxPriority = priorityMap.keySet().stream().max(Integer::compareTo).orElse(null);
                if (maxPriority == null)
                    return null;

                conflictTransitions = priorityMap.get(maxPriority);
            }

            aT = conflictTransitions.get(0);
            int i = 0;
            while (i < conflictTransitions.size() && conflictTransitions.get(i).getPriority() == aT.getPriority()) {
                i++;
            }
            if (i == 1)
             ; else {
                ...
                double sum = 0;
                double prob;
                while (j < conflictTransitions.size() && conflictTransitions.get(j).getPriority() == aT.getPriority()) {

                    if (conflictTransitions.get(j).getProbability() == 1.0) {
                        prob = 1.0 / i;
                    } else {
                        prob = conflictTransitions.get(j).getProbability();
                    }

                    sum = sum + prob;
                    if (r < sum) {
                        aT = conflictTransitions.get(j);
                        break;
...
//   public void resolveWbfConflict(){
//       PetriP petriP = Arrays.stream(listP).filter(PetriP::isJobQueue).findFirst().orElse(null);
//       if (petriP == null)
//           return;
//
//       List<UUID> uuids = Arrays.stream(listT).filter(t -> t.getTransitionType() == TransitionType.WORKBENCH).map(t -> t.getUuid()).collect(Collectors.toList());
//       Map<UUID, String> uuidToNameMap = Arrays.stream(listT)
//               .filter(t -> t.getTransitionType() == TransitionType.WORKBENCH)
//               .collect(Collectors.toMap(PetriT::getUuid, PetriT::getName));
//       List<WorkbenchFreeMarker> wbfMarkers = petriP.getMarkers().stream().map(m -> (WorkbenchFreeMarker) m).collect(Collectors.toList());
//       for (int i = 0; i < uuids.size(); i++) {
//           wbfMarkers.get(i).setWorkbenchUuid(uuids.get(i));
//           Utils.setTName(uuids.get(i), uuidToNameMap.get(uuids.get(i)));
//       }
//   }
