...
public class PetriT extends PetriMainElement implements Cloneable, Serializable {
   ...
   // private static double timeModeling = Double.MAX_VALUE - 1;
    ...
    public double getTimeServ() {
        double a = timeServ;
        if (distribution != null || transitionType == TransitionType.WORKBENCH) //додано 6 серпня
        {
        ...
        try {
            if (transitionType == TransitionType.WORKBENCH){
                ...
                if (distribution.equalsIgnoreCase("exp")) {
                    ...
                    timeServ = FunRand.norm(parametr, paramDeviation, false);// added 18.01.2013
                } else;
            } else {
     ...
     */
    public boolean condition(PetriP[] pp, Consumer<String> logAction) { //Нумерація позицій тут відносна!!!  inP.get(i) - номер позиції у списку позицій, який побудований при конструюванні мережі Петрі,
    ...
    public void actIn(PetriP[] pp, double currentTime, Consumer<String> logAction) {
        if (this.condition(pp, logAction) == true) {
            if (transitionType == TransitionType.JOB_FINISHED_CHECKER){
                ...
                if (jobMarker.getCurrentOperationsCount() <= 0)
                ...
                if (jobMarker.getCurrentOperationsCount() > 0)
                    logAction.accept(MessageFormat.format("\nJob {0} unfinished, returning to queue\n", jobMarker.getUuid()));
            }
            ...
            if (!allInputMarkers.isEmpty()) {
                markers.add(allInputMarkers.stream().filter(m -> m instanceof JobMarker).findFirst().orElse(allInputMarkers.get(0)));
            }
    ...
    private JobMarker getJobMarker(PetriP[] pp){
        ArrayList<Marker> allInputMarkers = new ArrayList<>();
        for (int i = 0; i < inP.size(); i++) {
            allInputMarkers.addAll(pp[inP.get(i)].getMarkersSlice(quantIn.get(i)));
        }

        Marker marker = allInputMarkers.stream().filter(m -> m instanceof JobMarker).findFirst().orElse(allInputMarkers.get(0));
        return marker instanceof JobMarker ? (JobMarker) marker : null;
    }

    public int getPriority(List<JobMarker> jobMarkers) {
        if (transitionType != TransitionType.WORKBENCH)
            return priority;

        JobMarker marker = jobMarkers.stream().filter(m -> !m.getUsedTransitionUuids().contains(uuid)).findFirst().orElse(null);
        if (marker == null)
            return 0;

        return jobMarkers.size() - jobMarkers.indexOf(marker);
    }
     ...
     * @param currentTime current time
     */
    public void actOut(PetriP[] pp, double currentTime, Consumer<String> logAction) {  // parameter current time ia added by Inna 11.07.2018 for protocol events
        if (buffer > 0) {
            Marker marker = Utils.popFirst(markers);
            if (marker instanceof JobMarker){
                if (transitionType == TransitionType.JOB_GENERATOR) {
                    JobMarker jobMarker = new JobMarker();
                    jobMarker.setOperationsCount(FunRand.generateOperationsCount());
                    jobMarker.setCreationTime(currentTime);
                    logAction.accept(MessageFormat.format("\nJob {0} generated: operations: {1}, completionTime: {2}, expectedFinishTime: {3}, creationTime: {4}\n\n",
                            jobMarker.getUuid(), jobMarker.getOperationsCount(), jobMarker.getExpectedCompletionTime(), jobMarker.getExpectedFinishTime(), jobMarker.getCreationTime()));
                    marker = jobMarker;
                }
                else if (transitionType == TransitionType.WORKBENCH){
                    JobMarker jobMarker = (JobMarker)marker;
                    jobMarker.getUsedTransitionUuids().add(uuid);
                    jobMarker.decreaseCurrentOperationsCount(currentTime);
                    logAction.accept(MessageFormat.format("\nJob {0} processed on workbench {1}, operations left: {2}, used transitions: [{3}], initial operations: {4}\n\n",
                            jobMarker.getUuid(), uuid, jobMarker.getCurrentOperationsCount(), jobMarker.getUsedTransitionUuids().stream().map(UUID::toString).collect(Collectors.joining(", ")), jobMarker.getOperationsCount()));
                }
            }

            for (int j = 0; j < getOutP().size(); j++) {
                int outQuantity = quantOut.get(j);
                PetriP petriP = pp[getOutP().get(j)];
                petriP.increaseMark(outQuantity, new ArrayList<>(getMarkersToAdd(marker, petriP, outQuantity)));
            }
    ...
    private ArrayList<Marker> getMarkersToAdd(Marker marker, PetriP petriP, int outQuantity){
        if (marker == null)
            return new ArrayList<>();

        List<Marker> result;
        if (transitionType == TransitionType.WORKBENCH){
            if (petriP.hasJobMarkerParameters() || petriP.isJobQueue()) {
                result = getDefaultMarkersToAdd(marker, outQuantity);
            } else {
                result = IntStream.range(0, outQuantity).mapToObj(i -> new Marker()).collect(Collectors.toList());
            }
        } else {
          result = getDefaultMarkersToAdd(marker, outQuantity);
        }
    ...
    private List<Marker> getDefaultMarkersToAdd(Marker marker, int outQuantity){
        return IntStream.range(0, outQuantity).mapToObj(i -> marker).collect(Collectors.toList());
    }
