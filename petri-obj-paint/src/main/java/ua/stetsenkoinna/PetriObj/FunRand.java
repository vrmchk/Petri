...
public  class FunRand {
     ...
     * @throws ExceptionInvalidTimeDelay the negative value of the delay time is excluded
     */
    public static double norm(double timeMean, double timeDeviation, boolean throwEx) throws ExceptionInvalidTimeDelay {
        double a;
        Random r = new Random();
    ...
    public static double workbench() {
        double expected = exp(Settings.getMeanProcessingTime());
        try {
            double additional = norm(Settings.getMeanNormProcessingTime(), Settings.getDeviationNormProcessing(), false);
            double result = Math.round(expected + additional);
    ...
    public static int generateOperationsCount() {
        ...
        do {
            try {
                operationCount = (int) Math.round(norm(mean, deviation, false));
            } catch (ExceptionInvalidTimeDelay e) {
                operationCount = 0;
            }
        } while (operationCount < minOperations || operationCount > maxOperations);
