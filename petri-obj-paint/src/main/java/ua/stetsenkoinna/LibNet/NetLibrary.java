...
public class NetLibrary {
    ...
    public static PetriNet workbenches() throws ExceptionInvalidTimeDelay {
        return createWorkbenches(25, 20, 0, 0.3);
    }

    @NetLibraryMethod
    public static PetriNet workbenchesVerifyGenerator() throws ExceptionInvalidTimeDelay {
        return createWorkbenches(10, 20, 0, 0.3);
    }

    private static PetriNet createWorkbenches(
            double generatorMeanDelay,
            double workbenchExpMeanDelay,
            double workbenchNormMeanDelay,
            double workbenchNormDeviation
    )
        ...
        final int workbenchesCount = 6;

        PetriP initial = new PetriP("");
        initial.setMark(1, true, false, false);
        places.add(initial);

        PetriT generator = new PetriT("Надходження робіт", generatorMeanDelay);
