...
public class AnimRunPetriSim extends PetriSim {
   ...
   public AnimRunPetriSim(PetriNet net,  JTextArea area, PetriNetsPanel panel, JSlider delaySlider, AnimRunPetriObjModel parentModel) {
    ...
    public void input(Consumer<String> logAction) {//вхід маркерів в переходи Петрі-об'єкта
        ...
        ArrayList<PetriT> activeT = this.findActiveT(super.getCurrentTime(), logAction);     //формування списку активних переходів
        ...
        if (activeT.isEmpty() && isBufferEmpty() == true) { //зупинка імітації за умови, що
            ...
            while (activeT.size() > 0) { //запуск переходів доки можливо
                PetriT tr = this.doConflikt(activeT, super.getNet().getListP(), super.getCurrentTime(), logAction);
                panel.animateP(tr.getInP());
                panel.animateIn(tr);
                tr.actIn(super.getNet().getListP(), super.getCurrentTime(), logAction); //розв'язання конфліктів
                panel.animateT(tr);
                doAfterStep();
                ...
                activeT = this.findActiveT(super.getCurrentTime(), logAction); //оновлення списку активних переходів
            }
    ...
    public void output(Consumer<String> logAction) {
        if (super.getCurrentTime() <= super.getSimulationTime()) {
            ...
            eventMin.actOut(super.getNet().getListP(),super.getCurrentTime(), logAction);//здійснення події
            ...
            if (eventMin.getBuffer() > 0) {
                ...
                while (u == true) {
                    ...
                    if (eventMin.getMinTime() == super.getCurrentTime()) {
                        panel.animateT(eventMin);
                        panel.animateOut(eventMin);
                        eventMin.actOut(super.getNet().getListP(),super.getCurrentTime(), logAction);
                        panel.animateP(eventMin.getOutP());
                        doAfterStep();
                        /* support for early termination of the simulation */
                        if (halted) {
            ...
            for (PetriT transition : super.getNet().getListT()) { //ВАЖЛИВО!!Вихід з усіх переходів, що час виходу маркерів == поточний момент час.
                ...
                if (transition.getBuffer() > 0 && transition.getMinTime() == super.getCurrentTime()) {
                    ...
                    transition.actOut(super.getNet().getListP(),super.getCurrentTime(), logAction);//Вихід маркерів з переходу, що відповідає найближчому моменту часу
                    ...
                    if (transition.getBuffer() > 0) {
                        ...
                        while (u == true) {
                            ...
                            if (transition.getMinTime() == super.getCurrentTime()) {
                                panel.animateT(transition); // 24.07.2018
                                panel.animateOut(transition); // 24.07.2018
                                transition.actOut(super.getNet().getListP(),super.getCurrentTime(), logAction);
                                panel.animateP(transition.getOutP()); // 24.07.2018
                            } else {
                                u = false;
                            }
    ...
    public void step(Consumer<String> logAction) //один крок,використовується для одного об'єкту мережа Петрі(наприклад, покрокова імітація мережі Петрі в графічному редакторі)
    {
        area.append("\n Next event, current time = " + getCurrentTime());

        this.printMark();//друкувати поточне маркування
        ArrayList<PetriT> activeT =  this.findActiveT(getCurrentTime(), logAction);     //формування списку активних переходів
        ...
        if ((activeT.isEmpty() && isBufferEmpty() == true) || getCurrentTime() >= getSimulationTime()) { //зупинка імітації за умови, що
          ...
          //  stop = true;                              // і не має фішок в переходах або вичерпаний час моделювання
            ...
            while (activeT.size() > 0) {      //вхід маркерів в переходи доки можливо
                ...
                PetriT petriT = this.doConflikt(activeT, super.getNet().getListP(), getCurrentTime(), logAction);
                area.append("\n Choosing a transition to activate " + petriT.getName());
                petriT.actIn(super.getNet().getListP(), getCurrentTime(), logAction); //розв'язання конфліктів
                doAfterStep();
                /* support for early termination of the simulation */
                if (halted) {
                    return;
                }
                activeT = this.findActiveT(getCurrentTime(), logAction); //оновлення списку активних переходів
            }
            ...
            if (getCurrentTime() <= getSimulationTime()) {
                ...
                //Вихід маркерів
                eventMin.actOut(super.getNet().getListP(),super.getCurrentTime(), logAction);//Вихід маркерів з переходу, що відповідає найближчому моменту часу
                doAfterStep();
                /* support for early termination of the simulation */
                if (halted) {
                    return;
