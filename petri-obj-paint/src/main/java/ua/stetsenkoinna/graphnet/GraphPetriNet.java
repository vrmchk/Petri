...
public class GraphPetriNet implements Cloneable, Serializable {
    ...
    public void printStatistics(JTextArea area) {
        ...
        for (GraphPetriPlace grP : graphPetriPlaceList) {
            ...
            if (P.isFinal())
            {
                area.append(MessageFormat.format("\nAverage job completion time = {0}\n", P.getAverageJobCompletionTime()));
                area.append(MessageFormat.format("\nAverage delay between creations = {0}\n", P.getAverageDelayBetweenCreations()));
                area.append(MessageFormat.format("\nCount of jobs finished late = {0}\n", P.getCountOfLateJobs()));
                area.append(MessageFormat.format("\nAverage delay = {0}\n", P.getAverageDelay()));
                List<JobMarker> jobMarkers = P.getMarkers().stream().map(m -> (JobMarker) m).collect(Collectors.toList());
                for (JobMarker marker : jobMarkers) {
                    area.append(MessageFormat.format("\nJob: {0}, with operations: {1}\n"
                            + "         creationTime: {2}\n"
                            + "         expectedCompletionTime: {3}\n"
                            + "         actualCompletionTime: {4}\n"
                            + "         expectedFinishTime: {5}\n"
                            + "         actualFinishTime: {6}\n",
                            marker.getUuid(),
                            marker.getOperationsCount(),
                            marker.getCreationTime(),
                            marker.getExpectedCompletionTime(),
                            marker.getActualCompletionTime(),
                            marker.getExpectedFinishTime(),
                            marker.getActualFinishTime()));
                }
            }
        }
